...
class C2f_LFCA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.attn= LFCA(c2)# optional act=FReLU(c2)
...
class LFCA(nn.Module):
    def __init__(self, ch, h_kernel_size=11, v_kernel_size=11) -> None:
        ...
        self.conv1 = GSConv(ch, ch)
        self.h_conv = nn.Conv2d(ch, ch, (1, h_kernel_size), 1, (0, h_kernel_size // 2), 1, ch)
        self.v_conv = nn.Conv2d(ch, ch, (v_kernel_size, 1), 1, (v_kernel_size // 2, 0), 1, ch)
        self.conv2 = GSConv(ch, ch)
        self.act = nn.Sigmoid()
...
class VoVGSCSPCLFCA(VoVGSCSP):
    ...
    def forward(self, x):
        x1 = self.gsb(self.cv1(x))
        y = self.cv2(x)
        return self.attn(self.cv3(torch.cat((y, x1), dim=1)))
